...
/**
 * Weekly insights via an OpenAI-compatible chat completions endpoint.
 *
 * Raw HTTP rather than a vendor SDK, for the same reason as the meal analyzer:
 * this driver also has to serve OpenAI-compatible gateways, which only agree on
 * the wire format.
 */
class OpenAiInsightGenerator implements NutritionInsightGenerator
{
    ...
    public function generate(WeeklyNutritionSummary $summary): array
    {
        ...
        try {
            $response = Http::withToken($apiKey)
                ...
                ->post($baseUrl.'/chat/completions', [
                    ...
                    'max_completion_tokens' => (int) config('ai.insights.max_tokens', 2000),
                    'response_format' => [
                        'type' => 'json_schema',
                        'json_schema' => [
                            'name' => 'weekly_insight',
                    ...
                    'messages' => [
                        ...
                        ['role' => 'user', 'content' => $this->prompt->userPrompt($summary)],
                    ],
                ]);
        ...
        if ($response->status() === 400 || $response->status() === 422) {
            Log::error('OpenAI-compatible endpoint rejected the insight request', [
                'status' => $response->status(),
        ...
        if ($response->failed()) {
            Log::warning('OpenAI-compatible weekly insight failed', ['status' => $response->status()]);
